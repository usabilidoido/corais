...
$(document).ready(function() {
  ...
  $('#menu-overview').parent().attr('id','od-menu-edit');
 });
...
Drupal.behaviors.outline_designer_book = function (context) {
  ...
  $('.outline_designer_edit_button').bind('click',function(e){
    Drupal.settings.outline_designer.activeMlid = $(this).attr('id').replace("-"," ").substring(5);
  });
  $("tr.draggable td").each(function(i){
    if((i+1) % 6 == 0 || (i+1) % 6 == 2 || (i+1) % 6 == 3 || (i+1) % 6 == 4 || (i+1) % 6 == 5 || (i+1) % 6 == 5) {
      this.style.display = 'none';
    }
  });  
  //replace text fields with span's w/ their same content
  $("#menu-overview .form-text", context).each(function(){
  ...
  $("#menu-overview span").bind('dblclick',function(e){
    Drupal.settings.outline_designer.activeMlid = $(this).attr('id').replace("edit-table-book-admin-","").replace("-title-span","");
    Drupal.outline_designer.form_render('rename');
  });
  //whenever you aren't active on a field, remove it
  $('#menu-overview input').blur(function(){
    $('#edit-table-book-admin-' + Drupal.settings.outline_designer.activeMlid + '-title-span').css('display','');
  $('#edit-table-book-admin-' + Drupal.settings.outline_designer.activeMlid + '-title').val($('#edit-table-book-admin-' + Drupal.settings.outline_designer.activeMlid + '-title-span').html());
  $('#edit-table-book-admin-' + Drupal.settings.outline_designer.activeMlid + '-title').css('display','none');
  });
  //if you hit enter, submit the title; if you hit esc then reset the field
  $('form').submit( function(){
   return false;
   } );
  $('#menu-overview input').keydown(function(e){
    if(document.all) {
      e = event;
    }
  ...
  //binding isn't working in Opera / IE correctly or at all
    $('.outline_designer_edit_button').contextMenu(Drupal.settings.outline_designer.context_menu, {theme: Drupal.settings.outline_designer.theme, beforeShow: function () { if ($.inArray("nid", unavailableContextMenuItems) == -1) { $(this.menu).find('.context-menu-item-inner:first').css('backgroundImage','url(' + $("#node-" + Drupal.settings.outline_designer.activeMlid +"-icon").attr('src') +')').empty().append("nid " + Drupal.settings.outline_designer.activeMlid);}
      },
      useIframe: false,
      shadow: false
    });
...
Drupal.outline_designer.get_active_title = function() {
  return $('#edit-table-book-admin-' + Drupal.settings.outline_designer.activeMlid + '-title-span').html();
};

Drupal.outline_designer.get_active_type = function() {
  return $("#node-" + Drupal.settings.outline_designer.activeMlid +"-icon").attr('alt');
};
...
Drupal.outline_designer.scale = function(scale){
  ...
  if(Drupal.settings.outline_designer.factor == 1){
    $("#menu-overview img").css('width','').css('height','');
    $("#menu-overview").css('font-size','');
    $("#menu-overview .form-item").css('margin-top','');
  }else{
    $("#menu-overview img").css('width',Drupal.settings.outline_designer.factor + 'em').css('height',Drupal.settings.outline_designer.factor + 'em');
    $("#menu-overview").css('font-size',Drupal.settings.outline_designer.factor + 'em');
    $("#menu-overview .form-item").css('margin-top',(Drupal.settings.outline_designer.factor/4) + 'em');
  }
};
...
Drupal.outline_designer.render_popup = function(render_title) {
  ...
  if (render_title == true) {
    ...
    output+= ' - <img src="'+  $("#node-" + Drupal.settings.outline_designer.activeMlid +"-icon").attr('src') +'" class="od_statusbar_img" />'+ title + ' (mlid:'+ Drupal.settings.outline_designer.activeMlid +')';
  }
  else {
...
// define function for edit
  Drupal.outline_designer_ops.edit = function() {
    window.open(Drupal.settings.basePath + '?q=admin/build/menu/item/' + Drupal.settings.outline_designer.activeMlid + '/edit','_blank');
  };
  // define function for view
  Drupal.outline_designer_ops.view = function() {
    window.open($('#mlid-'+ Drupal.settings.outline_designer.activeMlid +'-icon').next().next().attr('href'),'_blank');
  };
  // define function for rename
  Drupal.outline_designer_ops.rename = function() {
    $('#edit-table-book-admin-' + Drupal.settings.outline_designer.activeMlid + '-title-span').css('display','none');
    $('#edit-table-book-admin-' + Drupal.settings.outline_designer.activeMlid + '-title').css('display','');
    $('#edit-table-book-admin-' + Drupal.settings.outline_designer.activeMlid + '-title').focus();
  };
  ...
  Drupal.outline_designer_ops.rename_submit = function() {
    $('#edit-table-book-admin-' + Drupal.settings.outline_designer.activeMlid + '-title-span').css('display','');
    $('#edit-table-book-admin-' + Drupal.settings.outline_designer.activeMlid + '-title').css('display','none');
    if ($('#edit-table-book-admin-' + Drupal.settings.outline_designer.activeMlid + '-title-span').html() != $('#edit-table-book-admin-' + Drupal.settings.outline_designer.activeMlid + '-title').val()) {
      var title = $.param($('#edit-table-book-admin-' + Drupal.settings.outline_designer.activeMlid + '-title'));
      ...
      Drupal.outline_designer.ajax_call(Drupal.settings.outline_designer.type, 'rename', Drupal.settings.outline_designer.activeMlid, title, null);
    }  
  };
  // submit handler for change type
  Drupal.outline_designer_ops.change_type_submit = function() {
    Drupal.outline_designer.ajax_call(Drupal.settings.outline_designer.type, 'change_type', Drupal.settings.outline_designer.activeMlid, Drupal.settings.outline_designer.activeType, null, 'Drupal.outline_designer_ops.change_type_submit_success');
  };
  ...
  Drupal.outline_designer_ops.change_type_submit_success = function(msg) {
    ...
    if(msg == 0) {
      ...
      $("#node-" + Drupal.settings.outline_designer.activeMlid +"-icon").attr('src',Drupal.settings.basePath + Drupal.settings.outline_designer.types[Drupal.settings.outline_designer.activeType][1]);
  ...
  Drupal.outline_designer_ops.duplicate_submit = function() {
    ...
    for(var i=0; i<times_to_dup; i++) {
      ...
      // standard ajax call
      Drupal.outline_designer.ajax_call(Drupal.settings.outline_designer.type, 'duplicate', Drupal.settings.outline_designer.activeMlid, $('#od_duplicate_multiple:checked').length, tmp_dup_title);
  ...
  Drupal.outline_designer_ops.add_content_submit = function() {
    ...
    if (title == "") {
      ...
      Drupal.outline_designer.ajax_call(Drupal.settings.outline_designer.type, 'add_content', title, Drupal.settings.outline_designer.activeType, Drupal.settings.outline_designer.activeMlid, 'Drupal.outline_designer_ops.add_content_submit_success');
    }
  };
  ...
  Drupal.outline_designer_ops.add_content_submit_success = function(msg) {
    ...
    if ($.inArray('collapse-'+ Drupal.settings.outline_designer.activeMlid, Drupal.settings.outline_designer.collapseList) != -1) {
      Drupal.settings.outline_designer.collapseList.splice($.inArray('collapse-'+ Drupal.settings.outline_designer.activeMlid, Drupal.settings.outline_designer.collapseList), 1);
    }
    $("#reload_table").trigger('change');
    if(msg == 0) {
  ...
  Drupal.outline_designer_ops.delete_submit = function() {
    ...
    Drupal.outline_designer.ajax_call(Drupal.settings.outline_designer.type, 'delete', Drupal.settings.outline_designer.activeMlid, multiple, null);
  };
